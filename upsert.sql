...
WITH location_house_mapping AS (
    SELECT
        ...
        CASE
            WHEN h.name ILIKE '%' || l.name || '%' THEN 1
            WHEN EXISTS (SELECT 1 FROM UNNEST(h.seats) AS seat WHERE seat ILIKE '%' || l.name || '%') THEN 2
            WHEN EXISTS (SELECT 1 FROM UNNEST(h.titles) AS title WHERE title ILIKE '%' || l.name || '%') THEN 3
            WHEN l.summary ILIKE '%' || h.name || '%' THEN 4
            ELSE 5
    ...
    ON
        h.name ILIKE '%' || l.name || '%'
        OR EXISTS (SELECT 1 FROM UNNEST(h.seats) AS seat WHERE seat ILIKE '%' || l.name || '%')
        OR EXISTS (SELECT 1 FROM UNNEST(h.titles) AS title WHERE title ILIKE '%' || l.name || '%')
...
WITH location_house_mapping AS (
    SELECT
        ...
        CASE
            ...
            WHEN EXISTS (SELECT 1 FROM UNNEST(h.seats) AS seat
                         WHERE seat ILIKE '%' || l.name || '%'
                         AND seat ~* ('\m' || l.name || '\M')) THEN 2
            WHEN EXISTS (SELECT 1 FROM UNNEST(h.titles) AS title
                         WHERE title ILIKE '%' || l.name || '%'
                         AND title ~* ('\m' || l.name || '\M')) THEN 3
            ELSE 4
        END AS match_priority,
        COALESCE(
    ...
    ON
        ...
        OR EXISTS (SELECT 1 FROM UNNEST(h.seats) AS seat
                   WHERE seat ILIKE '%' || l.name || '%'
                   AND seat ~* ('\m' || l.name || '\M'))
        OR EXISTS (SELECT 1 FROM UNNEST(h.titles) AS title
                   WHERE title ILIKE '%' || l.name || '%'
